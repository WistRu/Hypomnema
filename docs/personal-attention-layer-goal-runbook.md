...
NEXT_READY_FRONTIER: configure the local Anthropic research provider without exposing secrets; run isolated real-public J3 against accepted R-UAT12-v1 through server egress, immutable handoff and final dossier; run the already approvable Compound captured-only preflight to a final fallback dossier; then perform user review before G9 PASS.
