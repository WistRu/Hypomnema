...
LAST_FULL_REGRESSION: 2026-08-24 at head 5469523 PASS through the mandated command itself, no substitution: `corepack pnpm test` exit 0 with shared 83/83, extension 342/342, MCP 140/140, server 1,338 passed and 2 skipped, web 631/631 — 2,534 passed and 2 skipped. `corepack pnpm typecheck` exit 0 and `corepack pnpm build` exit 0. This record is trustworthy in a way the previous one was not: the command was run four consecutive times and passed every time, after six consecutive clean runs of the server suite alone, and every run now verifies that it executed every test file on disk rather than reporting a total it did not earn (issue #36). The two skips are the live-database migration proof and its gate, which report themselves skipped unless TABHUB_PROVE_LIVE_MIGRATION=1. Evidence: docs/implementation-evidence/personal-attention-layer/server-suite-stability-table.md. NOT CLAIMED: the instability recorded on #36 the previous evening was never reproduced, so it was never diagnosed. The runs are clean; the cause is unknown and #36 stays open.
