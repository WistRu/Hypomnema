...
LAST_FULL_REGRESSION: 2026-08-23 after canonical-hash unification PASS: shared 78/78, extension 321/321, MCP 140/140, server 1300/1300 across 134 files and web 609/609 (2,448/2,448 total across 280 files) at head 409638d. Run per package with `npx vitest run --maxWorkers=4`: the workspace-wide `corepack pnpm test` times out tests at full concurrency that pass at bounded worker counts, which is pre-existing and not hash-related. `corepack pnpm typecheck` and `corepack pnpm build` exit 0. User runtime/browser/live DB untouched. Previous 2026-08-21 regression (2,435/2,435) is retained as audit history.
