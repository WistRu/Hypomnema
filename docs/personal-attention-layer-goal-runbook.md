...
NEXT_READY_FRONTIER: (1) collapse the four canonical-JSON/SHA-256 implementations into `researchCanonicalSha256PayloadV1` and re-run tests plus G8/G9 receipts; (2) fresh backup -> migrate a copy 17 -> 26 -> feature-off/read-only smoke (C98-style) -> migrate live DB -> start new server with context/resources/activity/logicalImportance/priority flags on and research/liveAcquisition/privacyPurge/pageSummaryCapture/priorityAssessmentWriter off; (3) Trial week = 7 days without rollback (triggers: data loss/corruption, Library slowdown, local-only text leaving the machine); (4) after the Trial week: remaining debt, then the pluggable-AI initiative (Agent write path first). Former frontier (provider config + real-public J3 + user review before G9 PASS) remains valid for G9 itself and is deferred.
...
LAST_FULL_REGRESSION: 2026-08-21 resulting schema-26/C90b/W90 state PASS: `corepack pnpm test` exit 0 with shared 75/75, extension 321/321, MCP 140/140, server 1290/1290 across 130 files and web 609/609 (2,435/2,435 total). `corepack pnpm typecheck`, `corepack pnpm build` and `git diff --check` exit 0. Server Vitest is stably bounded at maxWorkers=2. User runtime/browser/live DB untouched.
