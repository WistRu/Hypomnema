...
REDEPLOY_3_2026-08-23: third redeploy the same day, head 89e135e -> fe36d6a, still before the Trial week began counting. Carries automatic browser detection and the Library pairing banner. No schema change and no migration. Pre-redeploy backup `backups/tabhub-2026-08-23T17-05-29.024Z.sqlite` sha256=3ec29e79...f025adba, verified schema 26 / integrity ok / FK 0 / 2,269 tabs. Old PID 676880 stopped, new PID 904356 on 127.0.0.1:7717. Health schema 26, `/app/` 200, `/api/tabs` 200, feature projection unchanged, served bundle fetched from the running server and verified to carry the banner copy. Unlike the two earlier redeploys the main database file hash moved (6215daee... -> 8dff92c6...): the runtime ingests continuously and the restart checkpointed the WAL into the main file. That is ordinary write activity, not a migration. Receipt `docs/implementation-evidence/personal-attention-layer/trial-week-redeploy-3-receipt.json`, verdict PASS_WITH_UNPROVEN_ITEMS. DETECTION VERIFIED 2026-08-23 17:11-17:15Z for edge and yandex, not by a contrived test. The user reinstalled both extensions rather than reloading them, wiping `browser.storage.local` (installation ids changed: edge 56024a43 -> 5507a07a, yandex 0bcfb199 -> f5c71c21), so no explicit choice existed. `currentTabCommandScope` returns undefined without a browser identity and the relay cannot register a scope without one, yet both registered within seconds already labelled correctly — detection is the only thing that could have supplied the label. The user then opened the extension options in both and found the browser already correct; opening writes nothing, because the save path is bound to the select's `change` event. STILL UNVERIFIED: chrome, whose branch names Chrome by the absence of any other brand and is therefore the one most likely to mislabel. SIDE EFFECT: both browsers lost their personal-context pairing, because the pass is bound to the installation id; capability rows 56024a43 and 0bcfb199 are now orphaned and can never authenticate again. PAIRING BANNER VERIFIED 2026-08-23 17:19:48Z on edge: the Library offered pairing unprompted to the unpaired browser and one click created capability row 5507a07a with zero open challenges left, so the challenge was created, handed over the bridge and consumed inside one action, with no code shown and none typed. `last_used_at` is still null — issued but not yet spent, which the first personal-context read or write in J1 will change. Banner unverified on yandex and chrome. The pairing banner is likewise unseen, because the installed extensions predate the probe field it keys on. Trial week still gated on issue #12.
REDEPLOY_4_2026-08-23: fourth redeploy the same day, head fe36d6a -> d9193d4, still before the Trial week began counting. Carries the fixes for #35 (an AI ordering with nothing assessed no longer passes itself off as one) and #34 (the rules preview answers in words). The user chose to fix both before starting the week rather than live with them for seven days. No schema change and no migration. Pre-redeploy backup `backups/tabhub-2026-08-23T20-09-56.533Z.sqlite` sha256=0f518450...8bfbedc3, verified schema 26 / integrity ok / FK 0 / 2,278 tabs. Old PID 904356 stopped, new PID 432436. Health schema 26, `/app/` 200, feature projection unchanged, served bundle fetched from the running server and verified to carry both fixes. NO EXTENSION RELOAD NEEDED: the extension package is untouched and the only shared change reaching it is an exported type, erased at build; both extensions reconnected unaided at 20:10:11Z at protocol 5. Receipt `docs/implementation-evidence/personal-attention-layer/trial-week-redeploy-4-receipt.json`. UNPROVEN: neither fix has been seen by a user in a browser; criteria 1 and 2 of issue #12 were last measured against a runtime that no longer exists and are owed one fresh measurement before the week starts; chrome detection remains unchecked. #34 stays open for its version-control vocabulary; #35 records one reachable flag combination that still cannot warn.
ISSUE_12_CLOSED_2026-08-23T20:15Z: the Trial week is no longer gated. All six criteria settled against PID 432436 at head d9193d4, the runtime that carries the week; earlier measurements were re-taken because three redeploys had invalidated them. Criteria 1-3 met (edge and yandex at protocol 5; 21 windows / 740 tabs identical before and after an exact tab activation; MCP 32 tools answering). J1 PASS all six legs. J2 PASS all four legs. J4 PARTIAL and recorded as such, not rounded up: user importance passed and no-AI-retention-action passed, the My/AI/Recommended leg was walked and FAILED (which produced #35), the rules leg could not be walked at all (#34), and the assessment and feedback legs are structurally blocked by the provider staying off under the #2 waiver. Turning the writer on to satisfy the wording would contradict user story 6 of #2 and was not done. ROLLBACK TRIGGER EVALUATION as the ticket requires: neither #34 nor #35 is data loss/corruption, Library slowdown, or local-only text leaving the machine; DECISION: no Rollback, feature-off recipe not applied, no restore performed. Walking the journeys produced four defects invisible to all 2,503 automated tests because each concerns what the interface communicates rather than what it computes: #31 (fixed and shipped), #33 (open), #34 (half fixed and shipped), #35 (fixed and shipped). STILL UNPROVEN and not to be read otherwise: chrome browser detection, the priorityReaders-off combination recorded on #35, and activityDailyWriter which /api/features does not project.
TRIAL_WEEK_CANCELLED_2026-08-24: the user cancelled the Trial week and the build freeze. What it cost is one claim — "used for seven consecutive days without a Rollback" — and nothing else, because the Final Gate was already unreachable under the user's own provider waiver (G9 PENDING), not because of the week. What it was costing: the freeze had already been amended twice inside 24 hours, day 1 was lost to a host power cut, and the week's actual product was finding five interface defects (#31, #33, #34, #35, plus #37 from the outage) — every one of which could only be fixed by changing the code the freeze existed to hold still. The ceremony was fighting the work it was meant to validate. WHAT REPLACES IT: the runtime now records its own availability (issue #37), so "used on N days, and here are the outages" is a measured fact rather than a ceremony, and it survives deploys. The three Rollback triggers stay live and unchanged — data loss or corruption, Library slowdown, local-only text leaving the machine — and a Rollback remains feature-off on schema 26. NO BUILD IS FROZEN from this point: development moves normally and deploys need no amendment. Issue #13 is closed as cancelled rather than completed. G9 stays PENDING, the Final Gate stays unclaimed, and neither may be claimed on the strength of a Trial week that did not run.
TRIAL_WEEK_START_2026-08-24: the first counted day is the first full day of real use after issue #12 closed, i.e. 2026-08-24. Seven days without a Rollback. Frozen build for the week: head 4a90291, PID 6364 on 127.0.0.1:7717, schema 26. AMENDED 2026-08-24T00:09+03:00: the freeze was recorded as d9193d4, then moved to 4a90291 eight minutes into day 1, before any use that day, to carry the #33 fix. Stated rather than glossed because the local date had already rolled over: day 1 is 2026-08-24 and runs on 4a90291 from its first minutes, no day of real use occurred on d9193d4, so no counted day is split across builds and the seven-day count is not restarted. Receipt `docs/implementation-evidence/personal-attention-layer/trial-week-redeploy-5-receipt.json`. Triggers: data loss or corruption, Library slowdown, local-only text leaving the machine. The five waived flags stay off for the whole period and no Provider credential is configured. G9 stays PENDING and the Final Gate is not claimed. Next ticket: #13.
...
LAST_FULL_REGRESSION: 2026-08-24 at head 742fd74 PASS through the mandated command itself, no substitution: `corepack pnpm test` exit 0 with shared 83/83, extension 342/342, MCP 140/140, server 1,338 passed and 2 skipped, web 631/631 — 2,534 passed and 2 skipped. `corepack pnpm typecheck` exit 0 and `corepack pnpm build` exit 0. This record is trustworthy in a way the previous one was not: the mandated command was run ten consecutive times and passed every time — six on this head and four on its immediate predecessor — after six consecutive clean runs of the server suite alone, and every run now verifies that it executed every test file on disk rather than reporting a total it did not earn (issue #36). Free physical memory across the ten runs stayed between 66.1 GB and 74.3 GB of 127.2 GB. The two skips are the live-database migration proof and its gate, which report themselves skipped unless TABHUB_PROVE_LIVE_MIGRATION=1. Evidence: docs/implementation-evidence/personal-attention-layer/server-suite-stability-table.md. NOT CLAIMED: the instability recorded on #36 the previous evening was never reproduced, so it was never diagnosed. The runs are clean; the cause is unknown and #36 stays open.
