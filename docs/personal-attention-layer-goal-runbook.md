...
LAST_FULL_REGRESSION: 2026-08-23 at head 99a1802a48676d1ba809341fa31a36830a3b9495 PASS through the mandated command itself, no substitution: `corepack pnpm test` exit 0 with shared 83/83 across 13 files, extension 321/321 across 29, MCP 140/140 across 12, server 1,310 passed and 2 skipped across 135, web 609/609 across 92 — 2,463 passed and 2 skipped across 281 files. The two skips are the live-database migration proof and its gate, which report themselves skipped unless TABHUB_PROVE_LIVE_MIGRATION=1; that proof was run separately at this head and passed. `corepack pnpm typecheck` exit 0 and `corepack pnpm build` exit 0. Free physical memory before the run: 17.9 GB of 127.2 GB. This run covers the work the previous record did not: the round-trips through storage, the hardened SQL byte mirror and the single walk behind both byte contracts, the workspace-wide canonicalization guard, noUnusedLocals in every package including the extension, and the render-versus-refetch fix in web. The 2026-08-23 record at head 409638d, produced by a hand-run per-package loop, is retained as audit history and does not satisfy the rule.
