...
LAST_FULL_REGRESSION: 2026-08-23 after canonical-hash unification PASS: `corepack pnpm test` with shared 78/78, extension 321/321, MCP 140/140, server 1295/1295 across 132 files and web 609/609 (2,443/2,443 total across 278 files). Two web tests timed out under full workspace concurrency and passed on a bounded re-run; server Vitest remains stable only at bounded worker counts. `corepack pnpm typecheck` and `corepack pnpm build` exit 0. User runtime/browser/live DB untouched. Previous 2026-08-21 regression (2,435/2,435) is retained as audit history.
