...
В **Open tabs** можно выделять строки на одной или всех страницах, выбирать безопасные лишние exact-копии, один точный hostname или неактивные вкладки заданного возраста. Выбор всех результатов и smart-пресеты читают один транзакционный bulk-снимок, поэтому обновляющийся список не теряет строки между OFFSET-страницами. Единая панель выполняет Move, Close, Pin/Unpin, Mute/Unmute, Sleep и Reload в любой подключённой установке Chrome, Yandex Browser, Edge или другого Chromium-браузера. Все строки одной live-команды должны принадлежать одной точной browser/install/session; смешанный либо устаревший выбор блокируется целиком и никогда не исполняется частично. Для удалённого профиля Move показывает только его собственные окна и новое окно — идентификаторы окон между браузерами не смешиваются. Закрытие показывает живой preview и receipt. До пяти недавних операций закрытия в течение десяти минут остаются доступны для **Reopen closed tabs** даже после перезагрузки веб-страницы; Undo восстанавливает URL, актуальные на момент удаления порядок и поддерживаемые флаги из `chrome.storage.session` (историю переходов, формы и JavaScript-состояние восстановить нельзя). Жёсткого лимита количества целей у live-команд нет: executor обрабатывает весь проверенный выбор через ограниченный пул до восьми одновременных удалений и никогда не обрезает его молча.
