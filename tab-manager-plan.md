...
- Основное действие по заголовку в **Open tabs** активирует существующий физический экземпляр по `(browser, installationId, browserSessionId, tabId)`, восстанавливает свёрнутое окно и переводит фокус на него. До получения снимка текущей сессии расширения действие заблокировано, поэтому переиспользованный после перезапуска браузера или расширения `tabId` не может увести на другую вкладку. Оно никогда не открывает новую копию URL; каноническая карточка остаётся отдельным действием **Details**. Вкладки другой установки доступны для переключения только из TabHub, открытого в их браузере/профиле.

✅ Критерий: при 459 физических вкладках Chrome endpoint и **Open tabs** показывают 459 строк/общий счётчик, даже если канонических страниц меньше; обзор возвращает точные группы дублей; подтверждённое закрытие удаляет перепроверенные незакреплённые копии, включая активные; клик по управляемой строке фокусирует её исходную вкладку без создания новой.

## 14. Управление физическими вкладками и рабочие пространства

- Все live-действия идут через bridge v4 и единый session-scoped executor расширения. Web UI не получает браузерные разрешения и не может управлять другой установкой или устаревшей сессией.
