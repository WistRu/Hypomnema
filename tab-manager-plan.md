...
✅ Критерий: при 459 физических вкладках Chrome endpoint и **Open tabs** показывают 459 строк/общий счётчик, даже если канонических страниц меньше; обзор возвращает точные группы дублей; подтверждённое закрытие удаляет только перепроверенные неактивные и незакреплённые копии; клик по управляемой строке фокусирует её исходную вкладку без создания новой.
...
- Поддерживаются Move в текущее, другое или новое окно, Pin/Unpin, Mute/Unmute, Sleep, Reload и Close. Close и Reload требуют явного подтверждения; Close повторно проверяет полный URL и защищает active, pinned и вкладку TabHub. Частичный результат всегда возвращает succeeded/skipped/failed.
